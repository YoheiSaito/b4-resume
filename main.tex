...
可能となるケースが現れた. 
これは, 初歩的検討でありながら提案手法の妥当性の表れであると考えられる. 
一方で, exp1, 5, 7では従来法よりも成功率が落ちた. 
また, exp8では提案手法を用いても成功することができなかった. 
これはパッチの評価, 更新, 選択がいまだ洗礼されていないためであると考えられる. 

パッチの評価では距離の影響が大きく出てしまう. 
exp7,exp8で用いたhappyStandは円柱に近い形状をしているため, 
点群の腹部や台座の距離は回転していても近くなってしまう. 
そのため, 有効な収束を行うためのパッチが残すことができなかった. 
これを解決するためには距離を標準化することや形状による影響を大きくする
などして, 距離がパッチ評価にかかる影響を小さくする必要がある. 
選択法ではよりパッチの相対位置の距離を離して選択する手法を考案する必要がある. 
これは, 部分的な局所解に陥ることを防ぎ, 
パッチの更新によって残されるパッチが一部に集中することを防ぐのに有効であると思われる. 
また, exp1, 5, 7では収束が遅い試行のとき, 
